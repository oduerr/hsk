...
def synthesize_wav(api_key: str, text: str, model: str, voice: str, instructions: str) -> bytes:
    ...
    payload = {
        ...
        "input": text,
        "format": "wav",
        # key part: pass the teaching prompt
...
def main():
    ...
    # Write INSTRUCTIONS_DEFAULT to output directory
    ...
    # Load instructions (priority: inline > file > default)
    if args.instructions:
        ...
        instructions_text = p.read_text(encoding="utf-8")
    else:
        instructions_text = INSTRUCTIONS_DEFAULT
    ...
    print(f"[TTS] Using instructions: {'inline' if args.instructions else ('file' if args.instructions_file else 'default')}")
    ...
    for i, r in enumerate(rows):
        ...
        try:
            audio = synthesize_wav(api_key, text, args.model, args.voice, instructions_text)
